%% Through-the-Wall Radar Human Activity Recognition Script: Multi-Stream Large Deep Ensemble Model
...
%       Improved NASNet-Large networks are trained via transfer learning for
%           each data stream using a pre-saved model file.
%       A soft voting ensemble strategy fuses probability vectors to derive
%           the final classification result.
%   Output:
%       Trained neural network models, evaluation metrics, and SAVED confusion matrices.
%
% Files to process:
%   Datasets located in RW_RTM_Set, RW_DTM_Set, RW_RDM_Set, and RW_Feature_Set.
%   Pre-trained model file: pretrained_nasnetlarge.mat
...
% Define input size requirements for NASNet-Large network
...
for i = 1:Num_Modalities
    fprintf('\n-------------------------------------------------------------\n');
    fprintf('Step 2.%d: Training NASNet-Large for Modality: %s\n', i, Modality_Names(i));
    fprintf('-------------------------------------------------------------\n');
    
    % Prepare augmented datastores with resizing to 331x331
    aug_Train = augmentedImageDatastore(Input_Size, Data_Train{i}, ...
        'ColorPreprocessing', 'gray2rgb');
    aug_Val   = augmentedImageDatastore(Input_Size, Data_Val{i}, ...
        'ColorPreprocessing', 'gray2rgb');
    
    % Retrieve modified NASNet architecture from code construction
    lgraph = get_nasnet_architecture(Num_Classes);
...
hFig = figure('Name', 'Multi-Modal NASNet-Large Ensemble Results', 'Color', 'w', 'Position', [100, 100, 800, 600]);
...
% Send an email when the processing is finished
sender = 'joeybg@126.com';
password = 'RQhimNCYmTGW657f';
time_now = datetime('now');
str_now = datestr(time_now, 'yyyy-mm-dd HH:MM:SS');
...
sendmail(sender, 'MATLAB Program Finished!', ['The network model has finished training and validation in ' str_now '.']);

disp('Processing Completed.');

%% Core Function: Architecture Modification
function lgraph = get_nasnet_architecture(num_classes)
    % get_nasnet_architecture - Construct improved NASNet-Large architecture layer by layer
    fprintf('  > Constructing NASNet-Large architecture...\n');
    
    % Define an initial empty layer graph variable
    lgraph = layerGraph();
    
    % Construct the layers in sequence
    tempLayers = [
    ...
    tempLayers = CustomNASNetZeroPadding2dLayer("zero_padding2d_5",[0 1 0 1]);
    lgraph = addLayers(lgraph,tempLayers);
    ...
    tempLayers = CustomNASNetZeroPadding2dLayer("zero_padding2d_6",[0 1 0 1]);
    lgraph = addLayers(lgraph,tempLayers);
    ...
    tempLayers = CustomNASNetZeroPadding2dLayer("zero_padding2d_7",[0 1 0 1]);
    lgraph = addLayers(lgraph,tempLayers);
    ...
    tempLayers = CustomNASNetZeroPadding2dLayer("zero_padding2d_8",[0 1 0 1]);
    lgraph = addLayers(lgraph,tempLayers);
    ...
    tempLayers = [
        ...
        fullyConnectedLayer(num_classes,"Name","predictions")
    ...
    % Link the layers to finish layer graph
    ...
    disp('  > NASNet-Large architecture constructed and modified for custom classes.');
end