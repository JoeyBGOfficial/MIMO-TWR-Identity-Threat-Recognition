%% Script for Simulated RTF Point Set Generator
...
%
% Description: 
%   1. Generates synthetic radar point cloud datasets based on RTF-FPS method.
%   2. Follows the class structure (P1-P4, Gun/NoGun) of SimH dataset.
%   3. Processes: Echo Sim -> RTF Cube Construction -> Thresholding -> FPS.
%   4. Outputs .mat files containing 'Point_Cloud' (1024x3 matrix).
...
% Define Output Path
root_PointSet = 'Simulated_RTFPointSet';

% Define Classes
% Matches SimH_Dataset_Generator_PSNR.m structure
classes = struct(...
...
create_dirs(root_PointSet, classes);
...
for cls_idx = 1:length(classes)
    ...
    for grp_idx = 1:cls.Count
        ...
        rand_pos_x = 2.5 + (rand - 0.5) * 0.5; % Started further back (2.5m) for 2s duration
        rand_pos_y = 0 + (rand - 0.5) * 0.5;
        initial_position_xy = [rand_pos_x, rand_pos_y];
        
        % Vary walking angle (+- 15 degrees) around 180
        % Defaulting to 180 or away. 
        % Let's use 180 as standard.
        base_angle = 180; 
        ...
        for r = 1:Num_Range_Bins
            ...
            RTF_Cube(r, :, :) = abs(S_stft).'; % Range x Time x Doppler
        end
        ...
        P_range = Range_Axis(r_idx); P_range = P_range(:);
        ...
        save_path = fullfile(root_PointSet, cls.Name, filename);
        
        % Save variable 'Point_Cloud' [1024 x 3] -> (Range, Time, Doppler)
        Point_Cloud = Final_Points; 
        save(save_path, 'Point_Cloud');
